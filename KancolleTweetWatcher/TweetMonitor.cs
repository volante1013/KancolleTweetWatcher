...
namespace KancolleTweetWatcher
{
	public static class TweetMonitor
	{
		...
		[FunctionName("TweetMonitor")] // ñàèTåéójì˙ÇÃ7:00Ç…ãNìÆ
		public static async Task Run ([TimerTrigger(productionTimer)]TimerInfo myTimer, TraceWriter log)
		{
			...
			var maintenanceTweets = await apponly.Statuses.UserTimelineAsync(param).ContinueWith(tl => tl.Result.Where(t => t.Text.Contains("ÉÅÉìÉe")));
